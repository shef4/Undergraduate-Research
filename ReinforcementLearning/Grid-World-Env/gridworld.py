...
class GridWorld(object):
    ...
    def isTerminalState(self, state):
        ...
        #return state in self.stateSpacePlus if isTerminal
    
    def addMagicSquares(self, magicSquares):
    #def generate new row/colomn(self, magicSquares):
    ...
    def offGridMove(self, newState, oldState):
        ...
        # if we're trying to wrap around to next row
            ...
            return False
        #generate grid for env based off env position action

    def step(self, action):
        agentX, agentY = self.getAgentRowAndColumn()
        resultingState = self.agentPosition + self.actionSpace[action]
        #resultingState = for loop of array update and generated row/collum based of action + self.actionSpace[action]
        if resultingState in self.magicSquares.keys():
            resultingState = magicSquares[resultingState]

        reward = -1 if not self.isTerminalState(resultingState) else 0
        #reward = 1 for stayuuing alive for each step
        #reward += -1 if self.actionSpace[action] == Pefered Action else -2
        #reward = -100 if self.isTreeState(resultingState)
        ...
        #if not self.istree(resultingState, self.agentPosition):
        #   self.setState(resultingState)
        #   return resultingState, reward, \
        #          self.istree(resultingState), None
        #else:
        #   return self.agentPosition, reward, \
        #          "self.istree(self.agentPosition) should =true", None
